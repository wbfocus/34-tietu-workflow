...
**默认 5 秒丝滑轮播不要走这里。** 那条仍用 `finish_cards_media.py` → `轮播_YYYY-MM-DD_标题（主题）.mp4`。本模式只写 `橱窗_YYYY-MM-DD_标题（主题）.mp4`，不覆盖轮播。
...
8. 成片第一帧可拼 `封面/cover-3x4.png` → `橱窗_YYYY-MM-DD_标题（主题）（带封面）.mp4`（仍是 1080×1440，封面只占 1 帧）。
...
产出：`<贴图文件夹>/成品视频/橱窗_YYYY-MM-DD_标题（主题）.mp4`（**1080×1440**），例如 `橱窗_2026-09-11_经营分析落不到车间（刊物）.mp4`。带封面的成片为同名加「（带封面）」。
