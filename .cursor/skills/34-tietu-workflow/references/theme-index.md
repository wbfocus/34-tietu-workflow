...
选定主题后，在 `workspace/HTMLcards/` 下新建文件夹，**模式 + 日期 + 标题 + 主题**：

- 上滑 A：`上滑_YYYY-MM-DD_选题标题（深色）`
- 上滑 B：`上滑_YYYY-MM-DD_选题标题（浅色）`
- 上滑 C：`上滑_YYYY-MM-DD_选题标题（刊物）`
- 轮播 / 橱窗：把开头的「上滑」换成「轮播」或「橱窗」，主题括号同样。
- A+B 各一套：两个独立文件夹。
